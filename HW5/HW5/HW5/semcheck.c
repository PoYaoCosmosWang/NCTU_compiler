...
void verifyArithmeticOp(struct expr_sem *op1, OPERATOR operator, struct expr_sem *op2)
{
    ...
    { // need to consider type coercion
        ...
        {
			if(op1->pType->type==INTEGER_t )//i1 r2
			{
				fprintf(out,"fstore %d\n",nextIndex);
				fprintf(out,"i2f\n");
				fprintf(out,"fload %d\n",nextIndex);
			}
			else if(op2->pType->type==INTEGER_t)//r1 i2
			{
				fprintf(out,"i2f\n");
			}
        ...
        else
        {
            ...
            { // compare each parameter
                ...
                { // verify type
					...
					//need to do the fxxking type coecision
					int pushNum=0;
					struct expr_sem* p;
					//count pushNum
					for(p=exprPtr->next;p!=NULL;p=p->next,pushNum++);
					//fprintf(out,"pushNum is %d\n",pushNum);	
					int n;
					//for load 
					for(n=pushNum;n>0;--n)
					{
						int i;
						for(i=0,p=exprPtr;i<n;++i,p=p->next);
						
						//fprintf(out,"now N=%d\n",n);
						//type store
						if(p->pType->type==REAL_t)
						{
							fprintf(out,"f");
						}
						else//bool int
					...
					for(p=exprPtr->next;p!=NULL;p=p->next)
					{
            ...
            else
            {
				...
				fprintf(out,"invokestatic %s/%s(",name,id);
				struct PTypeList* ptr;
				for(ptr=node->attribute->formalParam->params;ptr!=NULL;ptr=ptr->next)
				{
				...
				struct SymNode* nowNode=lookupSymbol(table,id,0,__FALSE);
				char c;
